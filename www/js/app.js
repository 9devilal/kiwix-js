...
define(['jquery', 'zimArchiveLoader', 'uiUtil', 'cookies','abstractFilesystemAccess','q'],
 function($, zimArchiveLoader, uiUtil, cookies, abstractFilesystemAccess, Q) {
    ...
    /**
     * Reads a remote archive with given URL, and returns the response in a Promise for the requested file (blob)
        ...
        request.onreadystatechange = function () {
            if (request.readyState === 4) {
                if (request.status >= 200 && request.status < 300 || request.status === 0) {
                    ...
                    return Q.Promise.resolve(request.response);
                } else {
                    return Q.Promise.reject("HTTP status " + request.status + " when reading " + url);
                }
            }
        };
